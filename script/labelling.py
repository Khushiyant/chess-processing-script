    ...
    if(nb_moves == len(game_moves)):
        ...
        if((white_won and board.turn) or (not white_won and not board.turn)):
            ...
            for move in bad_moves:
                ...
                line = np.concatenate(
                    (board_features, from_square, to_square, list([False])))
                data.append(line)

            # append good move to data
            from_square, to_square = get_move_features(good_move)
            line = np.concatenate(
                (board_features, from_square, to_square, list([True])))
            data.append(line)
...
def process_game_data(direname, filename):
    ...
    columns = board_feature_names + move_from_feature_names + \
        move_to_feature_names + list(['good_move'])

    df = pd.DataFrame(data=data, columns=columns)

    new_filename = filename.replace('pgn', 'csv')
    new_dirname = 'resources\CSVs\CSV_TAL'
...
for dirname, _, filenames in os.walk('resources\datasets\Raw_game\Raw_game\Tal'):
